...
unitTest({perms: {read: true, write: true}}, function statSyncSuccess() {
  ...
  let now = Date.now();
  assert(tempInfo.atime !== null && now - tempInfo.atime.valueOf() < 1e3);
  assert(tempInfo.mtime !== null && now - tempInfo.mtime.valueOf() < 1e3);
  assert(tempInfo.birthtime === null || now - tempInfo.birthtime.valueOf() < 1e3);
  ...
  const modulesInfoByUrl = Deno.statSync(pathToAbsoluteFileUrl("cli/tests/symlink_to_subdir"));
  ...
  const tempInfoByUrl = Deno.statSync(new URL(`file://${Deno.build.os === "windows" ? "/" : ""}${tempFileForUrl}`));
  now = Date.now();
  assert(tempInfoByUrl.atime !== null && now - tempInfoByUrl.atime.valueOf() < 1e3);
  assert(tempInfoByUrl.mtime !== null && now - tempInfoByUrl.mtime.valueOf() < 1e3);
  assert(tempInfoByUrl.birthtime === null || now - tempInfoByUrl.birthtime.valueOf() < 1e3);
...
unitTest({perms: {read: true}}, function lstatSyncSuccess() {
  ...
  assert(!modulesInfo.isDirectory);
  assert(modulesInfo.isSymlink);
  const modulesInfoByUrl = Deno.lstatSync(pathToAbsoluteFileUrl("cli/tests/symlink_to_subdir"));
  assert(!modulesInfoByUrl.isDirectory);
  assert(modulesInfoByUrl.isSymlink);
...
unitTest({perms: {read: true, write: true}}, async function statSuccess() {
  const packageInfo = await Deno.stat("README.md");
  assert(packageInfo.isFile);
  assert(!packageInfo.isSymlink);
  const packageInfoByUrl = await Deno.stat(pathToAbsoluteFileUrl("README.md"));
  assert(packageInfoByUrl.isFile);
  assert(!packageInfoByUrl.isSymlink);
  const modulesInfo = await Deno.stat("cli/tests/symlink_to_subdir");
  assert(modulesInfo.isDirectory);
  assert(!modulesInfo.isSymlink);
  const modulesInfoByUrl = await Deno.stat(pathToAbsoluteFileUrl("cli/tests/symlink_to_subdir"));
  assert(modulesInfoByUrl.isDirectory);
  assert(!modulesInfoByUrl.isSymlink);
  ...
  const tempInfoByUrl = await Deno.stat(new URL(`file://${Deno.build.os === "windows" ? "/" : ""}${tempFileForUrl}`));
  now = Date.now();
  assert(tempInfoByUrl.atime !== null && now - tempInfoByUrl.atime.valueOf() < 1e3);
  assert(tempInfoByUrl.mtime !== null && now - tempInfoByUrl.mtime.valueOf() < 1e3);
  assert(tempInfoByUrl.birthtime === null || now - tempInfoByUrl.birthtime.valueOf() < 1e3);
...
unitTest({perms: {read: true}}, async function lstatSuccess() {
  ...
  assert(!modulesInfo.isDirectory);
  assert(modulesInfo.isSymlink);
  const modulesInfoByUrl = await Deno.lstat(pathToAbsoluteFileUrl("cli/tests/symlink_to_subdir"));
  assert(!modulesInfoByUrl.isDirectory);
  assert(modulesInfoByUrl.isSymlink);
...
unitTest({ignore: Deno.build.os !== "windows", perms: {read: true, write: true}}, function statNoUnixFields() {
...
unitTest({ignore: Deno.build.os === "windows", perms: {read: true, write: true}}, function statUnixFields() {
  const enc = new TextEncoder();
  const data = enc.encode("Hello");
  const tempDir = Deno.makeTempDirSync();
  const filename = tempDir + "/test.txt";
