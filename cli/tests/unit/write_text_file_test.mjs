...
unitTest({perms: {read: true, write: true}}, function writeTextFileSyncSuccess() {
  const filename = Deno.makeTempDirSync() + "/test.txt";
  Deno.writeTextFileSync(filename, "Hello");
  const dataRead = Deno.readTextFileSync(filename);
  assertEquals("Hello", dataRead);
});
unitTest({perms: {read: true, write: true}}, function writeTextFileSyncByUrl() {
...
unitTest({perms: {read: true, write: true}}, function writeTextFileSyncUpdateMode() {
  if (Deno.build.os !== "windows") {
    const data = "Hello";
    const filename = Deno.makeTempDirSync() + "/test.txt";
    Deno.writeTextFileSync(filename, data, {mode: 493});
    assertEquals(Deno.statSync(filename).mode & 511, 493);
    Deno.writeTextFileSync(filename, data, {mode: 438});
    assertEquals(Deno.statSync(filename).mode & 511, 438);
  }
});
unitTest({perms: {read: true, write: true}}, function writeTextFileSyncCreate() {
  const data = "Hello";
  const filename = Deno.makeTempDirSync() + "/test.txt";
...
unitTest({perms: {read: true, write: true}}, function writeTextFileSyncAppend() {
  const data = "Hello";
  const filename = Deno.makeTempDirSync() + "/test.txt";
...
unitTest({perms: {read: true, write: true}}, async function writeTextFileSuccess() {
  const filename = Deno.makeTempDirSync() + "/test.txt";
  await Deno.writeTextFile(filename, "Hello");
  const dataRead = Deno.readTextFileSync(filename);
  assertEquals("Hello", dataRead);
});
unitTest({perms: {read: true, write: true}}, async function writeTextFileByUrl() {
  const tempDir = Deno.makeTempDirSync();
  const fileUrl = new URL(`file://${Deno.build.os === "windows" ? "/" : ""}${tempDir}/test.txt`);
...
unitTest({perms: {read: true, write: true}}, async function writeTextFileNotFound() {
  const filename = "/baddir/test.txt";
  await assertThrowsAsync(async () => {
    await Deno.writeTextFile(filename, "Hello");
  }, Deno.errors.NotFound);
});
unitTest({perms: {write: false}}, async function writeTextFilePerm() {
  const filename = "/baddir/test.txt";
  await assertThrowsAsync(async () => {
    await Deno.writeTextFile(filename, "Hello");
  }, Deno.errors.PermissionDenied);
});
unitTest({perms: {read: true, write: true}}, async function writeTextFileUpdateMode() {
  if (Deno.build.os !== "windows") {
    const data = "Hello";
    const filename = Deno.makeTempDirSync() + "/test.txt";
    await Deno.writeTextFile(filename, data, {mode: 493});
    assertEquals(Deno.statSync(filename).mode & 511, 493);
    await Deno.writeTextFile(filename, data, {mode: 438});
    assertEquals(Deno.statSync(filename).mode & 511, 438);
  }
});
unitTest({perms: {read: true, write: true}}, async function writeTextFileCreate() {
  const data = "Hello";
  const filename = Deno.makeTempDirSync() + "/test.txt";
  let caughtError = false;
  try {
...
unitTest({perms: {read: true, write: true}}, async function writeTextFileAppend() {
  const data = "Hello";
  const filename = Deno.makeTempDirSync() + "/test.txt";
