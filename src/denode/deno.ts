 ...
 *      }
 */
export async function* toAsyncIterator(r: Reader): AsyncIterableIterator<Uint8Array> {
  let result: EOF | number;
  const buf = new Uint8Array(2048);
...
export interface DenoFile extends Reader, SyncReader, Writer, SyncWriter, Seeker, SyncSeeker, Closer {
  readonly rid: number;
}
 ...
 *       })();
 */
export async function open(filename: string, mode?: OpenMode): Promise<DenoFile> {
 ...
 */
export async function makeTempDir(options?: MakeTempDirOptions): Promise<string> {
  if (!options) {
 ...
 */
export function removeSync(path: string, options: RemoveOption = { recursive: false }): void {
  // TODO: recursive
 ...
 */
export function symlinkSync(oldname: string, newname: string, type?: "dir" | "file"): void {
 ...
 */
export function symlink(oldname: string, newname: string, type?: "file" | "dir"): Promise<void> {
 ...
 */
export const Signal: typeof MacOSSignal | typeof LinuxSignal = os.platform() === "darwin" ? MacOSSignal : LinuxSignal;
// export {};
