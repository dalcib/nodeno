...
Deno.test("[bytes] findIndex1", () => {
  const i = findIndex(new Uint8Array([1, 2, 0, 1, 2, 0, 1, 2, 0, 1, 3]), new Uint8Array([0, 1, 2]));
  assertEquals(i, 2);
});
...
Deno.test("[bytes] findLastIndex1", () => {
  const i = findLastIndex(new Uint8Array([0, 1, 2, 0, 1, 2, 0, 1, 3]), new Uint8Array([0, 1, 2]));
  assertEquals(i, 3);
});
...
Deno.test("[bytes] repeat", () => {
  ...
  for (const [input, output, count, errMsg] of repeatTestCase) {
    if (errMsg) {
      ...
      const newBytes = repeat(new TextEncoder().encode(input), count);
...
Deno.test("[io/tuil] copyBytes", function() {
