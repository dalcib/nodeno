...
import {decode, encode} from "./ascii85.mjs";
const utf8encoder = new TextEncoder();
const testCasesNoDelimeter = {
  Adobe: [
    ["test", "FCfN8"],
    ["ascii85", "@<5pmBfIs"],
...
for (const [standard, tests] of Object.entries(testCasesNoDelimeter)) {
  ...
  Deno.test({
    ...
    fn() {
      for (const [bin, b85] of tests) {
        assertEquals(encode(utf8encoder.encode(bin), {
          standard
        }), b85);
      }
    }
  ...
  Deno.test({
    ...
    fn() {
      for (const [bin, b85] of tests) {
        assertEquals(decode(b85, {standard}), utf8encoder.encode(bin));
      }
    }
  });
}
