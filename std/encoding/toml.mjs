import {deepAssign} from "../_util/deep_assign.mjs";
...
${line}`);
  ...
  _groupToOutput() {
    assert2(this.context.currentGroup != null, "currentGroup must be set");
  ...
  _createGroup(line) {
    ...
    assert2(m != null, "line mut be matched");
    let name = m[1];
    if (name.match(/\[.*\]/)) {
      type = "array";
      m = name.match(captureReg);
      assert2(m != null, "name must be matched");
      name = m[1];
    } else {
      type = "object";
    }
...
class Dumper {
  ...
  _parse(obj, keys = []) {
    ...
    for (let i = 0; i < k.length; i++) {
      ...
      } else if (value instanceof Array && this._isSimplySerializable(value[0])) {
        out.push(this._arrayDeclaration([prop], value));
      } else if (value instanceof Array && !this._isSimplySerializable(value[0])) {
        for (let i2 = 0; i2 < value.length; i2++) {
          out.push("");
          out.push(this._headerGroup([...keys, prop]));
          out.push(...this._parse(value[i2], [...keys, prop]));
        }
