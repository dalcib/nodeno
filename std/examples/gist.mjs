...
if (parsedArgs._.length === 0) {
  console.error("Usage: gist.ts --allow-env --allow-net [-t|--title Example] some_file [next_file]");
  Deno.exit(1);
}
const files = {};
for (const filename of parsedArgs._) {
  const base = pathBase(filename);
  const content2 = await Deno.readFile(filename);
  const contentStr = new TextDecoder().decode(content2);
