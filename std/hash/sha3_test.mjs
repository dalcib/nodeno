...
Deno.test("[hash/sha3] testSha3-224Raw", () => {
  const sha3sum = (data) => {
    const sha32 = new Sha3_224();
    return sha32.update(data).digest();
  };
...
Deno.test("[hash/sha3] testSha3-224String", () => {
  const sha3sum = (data) => {
    const sha32 = new Sha3_224();
    return sha32.update(data).toString();
  };
  for (const [input, output] of testSetSha3_224) {
...
Deno.test("[hash/sha3] testSha3-256Chain", () => {
  const sha32 = new Sha3_256();
  const output = sha32.update(s2b("a")).update(s2b("b")).update(s2b("c")).toString();
...
Deno.test("[hash/sha3] testSha3UpdateFinalized", () => {
  assertThrows(() => {
    const sha32 = new Sha3_256();
    const hash = sha32.update(s2b("a")).digest();
    const hash2 = sha32.update(s2b("a")).digest();
    assertEquals(hash, hash2);
  }, Error, "sha3: cannot update already finalized hash");
});
