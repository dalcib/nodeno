...
import {BaseHandler, FileHandler, RotatingFileHandler} from "./handlers.mjs";
import {LogRecord} from "./logger.mjs";
import {existsSync} from "../fs/exists.mjs";
...
Deno.test("testFormatterAsFunction", function() {
  const handler = new TestHandler("DEBUG", {
    formatter: (logRecord) => `fn formatter ${logRecord.levelName} ${logRecord.msg}`
  });
  handler.handle(new LogRecord({
    msg: "Hello, world!",
    args: [],
...
Deno.test({
  ...
  async fn() {
    ...
    fileHandler.handle(new LogRecord({
      msg: "Hello World",
      args: [],
...
Deno.test({
  name: "RotatingFileHandler with mode 'w' will wipe clean existing log file and remove others",
  async fn() {
    Deno.writeFileSync(LOG_FILE, new TextEncoder().encode("hello world"));
    Deno.writeFileSync(LOG_FILE + ".1", new TextEncoder().encode("hello world"));
    Deno.writeFileSync(LOG_FILE + ".2", new TextEncoder().encode("hello world"));
    Deno.writeFileSync(LOG_FILE + ".3", new TextEncoder().encode("hello world"));
...
Deno.test({
  name: "RotatingFileHandler with mode 'x' will throw if any log file already exists",
  async fn() {
    Deno.writeFileSync(LOG_FILE + ".3", new TextEncoder().encode("hello world"));
    const fileHandler = new RotatingFileHandler("WARNING", {
      filename: LOG_FILE,
    ...
    await assertThrowsAsync(async () => {
      await fileHandler.setup();
    }, Deno.errors.AlreadyExists, "Backup log file " + LOG_FILE + ".3 already exists");
    fileHandler.destroy();
...
Deno.test({
  ...
  async fn() {
    ...
    Deno.writeFileSync(LOG_FILE + ".1", new TextEncoder().encode("original log.1 file"));
    Deno.writeFileSync(LOG_FILE + ".2", new TextEncoder().encode("original log.2 file"));
    Deno.writeFileSync(LOG_FILE + ".3", new TextEncoder().encode("original log.3 file"));
    const fileHandler = new RotatingFileHandler("WARNING", {
      filename: LOG_FILE,
      maxBytes: 2,
      maxBackupCount: 3,
    ...
    assertEquals(decoder.decode(Deno.readFileSync(LOG_FILE + ".1")), "original log file");
    assertEquals(decoder.decode(Deno.readFileSync(LOG_FILE + ".2")), "original log.1 file");
    assertEquals(decoder.decode(Deno.readFileSync(LOG_FILE + ".3")), "original log.2 file");
...
Deno.test({
  ...
  async fn() {
    await assertThrowsAsync(async () => {
      ...
      await fileHandler.setup();
    }, Error, "maxBytes cannot be less than 1");
  }
});
Deno.test({
  ...
  async fn() {
    await assertThrowsAsync(async () => {
      ...
      await fileHandler.setup();
    }, Error, "maxBackupCount cannot be less than 1");
  }
});
Deno.test({
