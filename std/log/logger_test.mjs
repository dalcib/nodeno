import {assert, assertEquals} from "../testing/asserts.mjs";
import {Logger} from "./logger.mjs";
import {LogLevels} from "./levels.mjs";
import {BaseHandler} from "./handlers.mjs";
class TestHandler extends BaseHandler {
  constructor() {
    super(...arguments);
    this.messages = [];
    this.records = [];
  }
  handle(record) {
    this.records.push(record);
    super.handle(record);
  }
  log(str) {
    this.messages.push(str);
  }
}
Deno.test({
  name: "Logger names can be output in logs",
  fn() {
    const handlerNoName = new TestHandler("DEBUG");
    const handlerWithLoggerName = new TestHandler("DEBUG", {
      formatter: "[{loggerName}] {levelName} {msg}"
    });
    const logger2 = new Logger("config", "DEBUG", {
      handlers: [handlerNoName, handlerWithLoggerName]
    });
    logger2.debug("hello");
    assertEquals(handlerNoName.messages[0], "DEBUG hello");
    assertEquals(handlerWithLoggerName.messages[0], "[config] DEBUG hello");
  }
});
Deno.test("simpleLogger", function() {
  const handler = new TestHandler("DEBUG");
  let logger2 = new Logger("default", "DEBUG");
  assertEquals(logger2.level, LogLevels.DEBUG);
  assertEquals(logger2.levelName, "DEBUG");
  assertEquals(logger2.handlers, []);
  logger2 = new Logger("default", "DEBUG", {handlers: [handler]});
  assertEquals(logger2.handlers, [handler]);
});
Deno.test("customHandler", function() {
  const handler = new TestHandler("DEBUG");
  const logger2 = new Logger("default", "DEBUG", {handlers: [handler]});
  const inlineData = logger2.debug("foo", 1, 2);
  const record = handler.records[0];
  assertEquals(record.msg, "foo");
  assertEquals(record.args, [1, 2]);
  assertEquals(record.level, LogLevels.DEBUG);
  assertEquals(record.levelName, "DEBUG");
  assertEquals(handler.messages, ["DEBUG foo"]);
  assertEquals(inlineData, "foo");
});
Deno.test("logFunctions", function() {
  const doLog = (level) => {
    const handler2 = new TestHandler(level);
    const logger2 = new Logger("default", level, {handlers: [handler2]});
    const debugData = logger2.debug("foo");
    const infoData = logger2.info("bar");
    const warningData = logger2.warning("baz");
    const errorData = logger2.error("boo");
    const criticalData = logger2.critical("doo");
    assertEquals(debugData, "foo");
    assertEquals(infoData, "bar");
    assertEquals(warningData, "baz");
    assertEquals(errorData, "boo");
    assertEquals(criticalData, "doo");
    return handler2;
  };
  let handler;
  handler = doLog("DEBUG");
  assertEquals(handler.messages, [
    "DEBUG foo",
    "INFO bar",
    "WARNING baz",
    "ERROR boo",
    "CRITICAL doo"
  ]);
  handler = doLog("INFO");
  assertEquals(handler.messages, [
    "INFO bar",
    "WARNING baz",
    "ERROR boo",
    "CRITICAL doo"
  ]);
  handler = doLog("WARNING");
  assertEquals(handler.messages, ["WARNING baz", "ERROR boo", "CRITICAL doo"]);
  handler = doLog("ERROR");
  assertEquals(handler.messages, ["ERROR boo", "CRITICAL doo"]);
  handler = doLog("CRITICAL");
  assertEquals(handler.messages, ["CRITICAL doo"]);
});
Deno.test("String resolver fn will not execute if msg will not be logged", function() {
  const handler = new TestHandler("ERROR");
  const logger2 = new Logger("default", "ERROR", {handlers: [handler]});
  let called = false;
  const expensiveFunction = () => {
    called = true;
    return "expensive function result";
  };
  const inlineData = logger2.debug(expensiveFunction, 1, 2);
  assert(!called);
  assertEquals(inlineData, void 0);
});
Deno.test("String resolver fn resolves as expected", function() {
  const handler = new TestHandler("ERROR");
  const logger2 = new Logger("default", "ERROR", {handlers: [handler]});
  const expensiveFunction = (x) => {
    return "expensive function result " + x;
  };
  const firstInlineData = logger2.error(() => expensiveFunction(5));
  const secondInlineData = logger2.error(() => expensiveFunction(12), 1, "abc");
  assertEquals(firstInlineData, "expensive function result 5");
  assertEquals(secondInlineData, "expensive function result 12");
});
Deno.test("All types map correctly to log strings and are returned as is", function() {
  const handler = new TestHandler("DEBUG");
  const logger2 = new Logger("default", "DEBUG", {handlers: [handler]});
  const sym = Symbol();
  const syma = Symbol("a");
  const fn = () => {
    return "abc";
  };
  const data1 = logger2.debug("abc");
  assertEquals(data1, "abc");
  const data2 = logger2.debug("def", 1);
  assertEquals(data2, "def");
  assertEquals(handler.messages[0], "DEBUG abc");
  assertEquals(handler.messages[1], "DEBUG def");
  const data3 = logger2.info(null);
  assertEquals(data3, null);
  const data4 = logger2.info(null, 1);
  assertEquals(data4, null);
  assertEquals(handler.messages[2], "INFO null");
  assertEquals(handler.messages[3], "INFO null");
  const data5 = logger2.warning(3);
  assertEquals(data5, 3);
  const data6 = logger2.warning(3, 1);
  assertEquals(data6, 3);
  assertEquals(handler.messages[4], "WARNING 3");
  assertEquals(handler.messages[5], "WARNING 3");
  const data7 = logger2.error(5n);
  assertEquals(data7, 5n);
  const data8 = logger2.error(5n, 1);
  assertEquals(data8, 5n);
  assertEquals(handler.messages[6], "ERROR 5");
  assertEquals(handler.messages[7], "ERROR 5");
  const data9 = logger2.critical(true);
  assertEquals(data9, true);
  const data10 = logger2.critical(false, 1);
  assertEquals(data10, false);
  assertEquals(handler.messages[8], "CRITICAL true");
  assertEquals(handler.messages[9], "CRITICAL false");
  const data11 = logger2.debug(void 0);
  assertEquals(data11, void 0);
  const data12 = logger2.debug(void 0, 1);
  assertEquals(data12, void 0);
  assertEquals(handler.messages[10], "DEBUG undefined");
  assertEquals(handler.messages[11], "DEBUG undefined");
  const data13 = logger2.info(sym);
  assertEquals(data13, sym);
  const data14 = logger2.info(syma, 1);
  assertEquals(data14, syma);
  assertEquals(handler.messages[12], "INFO Symbol()");
  assertEquals(handler.messages[13], "INFO Symbol(a)");
  const data15 = logger2.warning(fn);
  assertEquals(data15, "abc");
  const data16 = logger2.warning(fn, 1);
  assertEquals(data16, "abc");
  assertEquals(handler.messages[14], "WARNING abc");
  assertEquals(handler.messages[15], "WARNING abc");
  const data17 = logger2.error({
    payload: "data",
    other: 123
  });
  assertEquals(data17, {
    payload: "data",
    other: 123
  });
  const data18 = logger2.error({payload: "data", other: 123}, 1);
  assertEquals(data18, {
    payload: "data",
    other: 123
  });
  assertEquals(handler.messages[16], 'ERROR {"payload":"data","other":123}');
  assertEquals(handler.messages[17], 'ERROR {"payload":"data","other":123}');
});
