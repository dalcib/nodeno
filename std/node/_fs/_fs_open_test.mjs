...
import {open, openSync} from "./_fs_open.mjs";
import {join, parse} from "../../path/mod.mjs";
import {existsSync} from "../../fs/mod.mjs";
import {closeSync} from "./_fs_close.mjs";
...
Deno.test({
  ...
  async fn() {
    ...
    }).catch(() => fail()).finally(() => closeSync(fd1));
  }
});
Deno.test({
  name: "SYNC: open file",
...
Deno.test({
  ...
  fn() {
    ...
    assertThrows(() => {
      openSync(file, "ax");
    }, Error, `EEXIST: file already exists, open '${file}'`);
...
Deno.test({
  ...
  fn() {
    ...
    assertThrows(() => {
      openSync(file2, "wx");
    }, Error, `EEXIST: file already exists, open '${file2}'`);
  }
});
Deno.test({
  name: "open with flag 'w+'",
