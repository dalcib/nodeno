...
import {readdir, readdirSync} from "./_fs_readdir.mjs";
import {join} from "../../path/mod.mjs";
Deno.test({
  ...
  async fn() {
    ...
    }).then((files) => assertEquals(files, [])).catch(() => fail()).finally(() => Deno.removeSync(dir));
  }
});
Deno.test({
  ...
  async fn() {
    ...
    }).then((files) => assertEquals(files, ["file1.txt", "some_dir", "file2.txt"])).catch(() => fail()).finally(() => Deno.removeSync(dir, {recursive: true}));
  }
});
Deno.test({
