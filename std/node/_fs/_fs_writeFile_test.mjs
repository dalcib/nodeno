...
import {writeFile, writeFileSync} from "./_fs_writeFile.mjs";
...
Deno.test("Data is written to correct file encodings", async function testCorrectWriteUsingDifferentEncodings() {
  const encodings = [
  ...
  for (const [encoding, value] of encodings) {
    const res = await new Promise((resolve) => {
      writeFile("_fs_writeFile_test_file.txt", value, encoding, resolve);
    });
    const data = await Deno.readFile("_fs_writeFile_test_file.txt");
    await Deno.remove("_fs_writeFile_test_file.txt");
    assertEquals(res, null);
...
Deno.test("Path can be an URL", async function testCorrectWriteUsingURL() {
  const url = new URL(Deno.build.os === "windows" ? "file:///" + path.join(testDataDir, "_fs_writeFile_test_file_url.txt").replace(/\\/g, "/") : "file://" + path.join(testDataDir, "_fs_writeFile_test_file_url.txt"));
...
Deno.test("Mode is correctly set", async function testCorrectFileMode() {
  ...
  const res = await new Promise((resolve) => {
    writeFile(filename, "hello world", {mode: 511}, resolve);
  });
...
Deno.test("Mode is not set when rid is passed", async function testCorrectFileModeRid() {
  ...
  await new Promise((resolve, reject) => {
    writeFile(file.rid, "hello world", {mode: 511}, (err) => {
      if (err)
        return reject(err);
      resolve();
    });
...
Deno.test("Data is written synchronously to correct rid", function testCorrectWriteSyncUsingRid() {
...
Deno.test("Data is written to correct file encodings", function testCorrectWriteSyncUsingDifferentEncodings() {
  const encodings = [
    ["hex", "68656c6c6f20776f726c64"],
    ["HEX", "68656c6c6f20776f726c64"],
    ["base64", "aGVsbG8gd29ybGQ="],
...
Deno.test("Data is written synchronously to correct file", function testCorrectWriteSyncUsingPath() {
  const file = "_fs_writeFileSync_test_file";
...
Deno.test("sync: Path can be an URL", function testCorrectWriteSyncUsingURL() {
  const filePath = path.join(testDataDir, "_fs_writeFileSync_test_file_url.txt");
  const url = new URL(Deno.build.os === "windows" ? "file:///" + filePath.replace(/\\/g, "/") : "file://" + filePath);
  writeFileSync(url, "hello world");
  const data = Deno.readFileSync(filePath);
  Deno.removeSync(filePath);
  assertEquals(decoder.decode(data), "hello world");
});
Deno.test("Mode is correctly set when writing synchronously", function testCorrectFileModeSync() {
  if (Deno.build.os === "windows")
    return;
  const filename = "_fs_writeFileSync_test_file.txt";
  writeFileSync(filename, "hello world", {mode: 511});
