import {stripColor} from "../fmt/colors.mjs";
import {
  assert,
  assertEquals,
...
Deno.test({
  ...
  fn() {
    ...
    assertNotStrictEquals(copy, err, "Copy should not be strictly equal to the original error.");
  }
});
Deno.test({
  name: "inspectValue()",
  fn() {
    ...
    assertStrictEquals(stripColor(inspectValue(obj)), "{ a: 1, b: [ 2 ] }");
  }
});
Deno.test({
  name: "createErrDiff()",
  fn() {
    assertStrictEquals(stripColor(createErrDiff({a: 1, b: 2}, {a: 2, b: 2}, "strictEqual")), stripColor('Expected "actual" to be reference-equal to "expected":\n+ actual - expected\n\n+ { a: 1, b: 2 }\n- { a: 2, b: 2 }'));
  }
});
Deno.test({
...
Deno.test({
  ...
  fn() {
    const err = new AssertionError({actual: 1, expected: 2, operator: "equal"});
    assertStrictEquals(err.name, "AssertionError");
...
Deno.test({
  ...
  fn: function stackStartFn() {
    ...
    assert(!err.stack?.includes("stackStartFn"), "stackStartFn() should not present in stack trace");
  }
});
Deno.test({
  name: "error details",
  fn() {
    ...
    const err = new AssertionError({
      ...
      details: [
        {
          message: "Expected the calls function to be executed 2 time(s) but was executed 3 time(s).",
          actual: 3,
          expected: 2,
          operator: "calls",
          stack: stack0
        ...
        {
          message: "Expected the fn function to be executed 1 time(s) but was executed 0 time(s).",
          actual: 0,
          expected: 1,
          operator: "fn",
          stack: stack1
    ...
    assertStrictEquals(err["message 0"], "Expected the calls function to be executed 2 time(s) but was executed 3 time(s).");
    ...
    assertStrictEquals(err["message 1"], "Expected the fn function to be executed 1 time(s) but was executed 0 time(s).");
