...
Deno.test({
  ...
  fn() {
    assertThrows(() => {
      Buffer.alloc(-1);
    }, RangeError, "Invalid typed array length: -1", "should throw on negative numbers");
  }
});
Deno.test({
  ...
  fn() {
    ...
    for (const size of invalidSizes) {
      assertThrows(() => {
        Buffer.alloc(size);
      }, TypeError, `The "size" argument must be of type number. Received type ${typeof size}`, "should throw on non-number size");
    }
  }
});
Deno.test({
  ...
  fn() {
    ...
    for (const value of invalidValues) {
      ...
      }, TypeError, `The argument "value" is invalid. Received ${value.constructor.name} []`, "should throw for empty Buffer/Uint8Array");
    }
  }
});
Deno.test({
...
Deno.test({
  ...
  fn() {
    ...
    assertEquals(buffer2.length, 1, "Buffer size should be 1");
    assertEquals(buffer2[0], 0, "Content should be filled with 0");
  }
});
Deno.test({
  ...
  fn() {
    ...
    assertEquals(buffer2.length, 0, "Buffer size should be 0");
  }
});
Deno.test({
...
Deno.test({
  ...
  fn() {
    const buffer2 = Buffer.allocUnsafe(0);
    assertEquals(buffer2.length, 0, "Buffer size should be 0");
  }
});
Deno.test({
  ...
  fn() {
    const buffer2 = Buffer.alloc(3, 5);
    assertEquals(buffer2, new Uint8Array([5, 5, 5]));
  }
});
Deno.test({
...
Deno.test({
  ...
  fn() {
    assertEquals(Buffer.alloc(11, "aGVsbG8gd29ybGQ=", "base64"), new Uint8Array([104, 101, 108, 108, 111, 32, 119, 111, 114, 108, 100]));
  }
});
Deno.test({
  name: "alloc filled correctly with hex string",
  fn() {
    assertEquals(Buffer.alloc(4, "64656e6f", "hex"), new Uint8Array([100, 101, 110, 111]));
  }
});
Deno.test({
  name: "alloc filled correctly with hex string smaller than alloc size",
  fn() {
    assertEquals(Buffer.alloc(13, "64656e6f", "hex").toString(), "denodenodenod");
  }
});
Deno.test({
  name: "alloc filled correctly with Uint8Array smaller than alloc size",
  fn() {
    assertEquals(Buffer.alloc(7, new Uint8Array([100, 101])), new Uint8Array([100, 101, 100, 101, 100, 101, 100]));
    assertEquals(Buffer.alloc(6, new Uint8Array([100, 101])), new Uint8Array([100, 101, 100, 101, 100, 101]));
  }
});
Deno.test({
  name: "alloc filled correctly with Uint8Array bigger than alloc size",
  fn() {
    assertEquals(Buffer.alloc(1, new Uint8Array([100, 101])), new Uint8Array([100]));
  }
});
Deno.test({
  name: "alloc filled correctly with Buffer",
  fn() {
    assertEquals(Buffer.alloc(6, new Buffer([100, 101])), new Uint8Array([100, 101, 100, 101, 100, 101]));
    assertEquals(Buffer.alloc(7, new Buffer([100, 101])), new Uint8Array([100, 101, 100, 101, 100, 101, 100]));
  }
});
Deno.test({
  ...
  fn() {
    ...
    assertEquals(Buffer.byteLength("bHNrZGZsa3NqZmtsc2xrZmFqc2RsZmtqcw==", "base64"), 25);
    ...
    ["ascii", "latin1", "binary"].reduce((es, e) => es.concat(e, e.toUpperCase()), []).forEach((encoding) => {
      assertEquals(Buffer.byteLength("Il était tué", encoding), 12);
    });
    ["ucs2", "ucs-2", "utf16le", "utf-16le"].reduce((es, e) => es.concat(e, e.toUpperCase()), []).forEach((encoding) => {
...
Deno.test({
  ...
  fn() {
    assertEquals(Buffer.byteLength(Buffer.alloc(0)), Buffer.alloc(0).byteLength, "Byte lenght differs on buffers");
  }
});
Deno.test({
...
Deno.test({
  ...
  fn() {
    ...
    }, RangeError, "offset is out of bounds", "should throw on negative numbers");
  }
});
Deno.test({
  ...
  fn() {
    ...
    assertEquals(buffer2.toString(), "test", "Buffer to string should recover the string");
  }
});
Deno.test({
  ...
  fn() {
    for (const encoding of ["hex", "HEX"]) {
      const buffer2 = Buffer.from("7468697320697320612074c3a97374", encoding);
      assertEquals(buffer2.length, 15, "Buffer length should be 15");
      assertEquals(buffer2.toString(), "this is a tést", "Buffer to string should recover the string");
    }
  }
});
Deno.test({
  ...
  fn() {
    for (const encoding of ["base64", "BASE64"]) {
      const buffer2 = Buffer.from("dGhpcyBpcyBhIHTDqXN0", encoding);
      assertEquals(buffer2.length, 15, "Buffer length should be 15");
      assertEquals(buffer2.toString(), "this is a tést", "Buffer to string should recover the string");
    }
  }
...
Deno.test({
  ...
  fn() {
    for (const encoding of ["base64", "BASE64"]) {
      const buffer2 = Buffer.from("deno land");
      assertEquals(buffer2.toString(encoding), "ZGVubyBsYW5k", "Buffer to string should recover the string in base64");
    }
...
Deno.test({
  ...
  fn() {
    for (const encoding of ["hex", "HEX"]) {
      const buffer2 = Buffer.from("deno land");
      assertEquals(buffer2.toString(encoding), "64656e6f206c616e64", "Buffer to string should recover the string");
    }
...
Deno.test({
  ...
  fn() {
    ...
    for (const encoding of invalidEncodings) {
      assertThrows(() => {
        buffer2.toString(encoding);
      }, TypeError, `Unkown encoding: ${encoding}`, "Should throw on invalid encoding");
    }
  }
});
Deno.test({
  ...
  fn() {
    ...
    for (const encoding of invalidEncodings) {
      ...
      }, TypeError, `Unkown encoding: ${encoding}`);
    }
  }
});
Deno.test({
  ...
  fn() {
    ...
    for (const encoding of notImplemented) {
      assertThrows(() => {
        buffer2.toString(encoding);
      }, Error, `"${encoding}" encoding`, "Should throw on invalid encoding");
      assertThrows(() => {
...
Deno.test({
  ...
  fn() {
    const buffer2 = Buffer.from(Buffer.from("test"));
    assertEquals(buffer2.length, 4, "Buffer length should be 4");
    assertEquals(buffer2.toString(), "test", "Buffer to string should recover the string");
  }
});
...
Deno.test({
  name: "Buffer toJSON",
  fn() {
    assertEquals(JSON.stringify(Buffer.from("deno")), '{"type":"Buffer","data":[100,101,110,111]}');
  }
});
Deno.test({
...
Deno.test({
  ...
  fn() {
    ...
    assertEquals(d.equals(new Uint8Array([97, 98, 99, 100, 101])), true);
    assertEquals(c.equals(d), false);
    assertEquals(d.equals(e), false);
    assertThrows(() => Buffer.alloc(1).equals("abc"), TypeError, `The "otherBuffer" argument must be an instance of Buffer or Uint8Array. Received type string`);
  }
});
