...
import EventEmitter, {on, once} from "./events.mjs";
...
Deno.test({
  ...
  fn() {
    ...
    console.warn = () => fail("Infinity listeners should be allowed");
...
Deno.test({
  ...
  fn() {
    const testEmitter = new EventEmitter();
    testEmitter.on("event", shouldNeverBeEmitted).on("event", shouldNeverBeEmitted);
...
Deno.test({
  ...
  fn() {
    const testEmitter = new EventEmitter();
    let eventsProcessed = [];
    const listenerB = () => eventsProcessed.push("B");
    const listenerA = () => {
...
Deno.test({
  ...
  fn() {
    ...
    const listenerA = () => eventsProcessed.push("A");
    const listenerB = () => eventsProcessed.push("B");
    testEmitter.on("event", listenerA);
...
Deno.test({
  ...
  fn() {
    ...
    const rawListenersForOnceEvent = testEmitter.rawListeners("once-event");
    const wrappedFn = rawListenersForOnceEvent[0];
    wrappedFn.listener();
    wrappedFn.listener();
    wrappedFn.listener();
...
Deno.test({
  ...
  async fn() {
    ...
    const valueArr = await once(ee, "event");
...
Deno.test({
  ...
  async fn() {
    ...
    setTimeout(() => {
      const event = new Event("event", {composed: true});
      et.dispatchEvent(event);
    }, 0);
    const eventObj = await once(et, "event");
...
Deno.test({
  ...
  fn() {
    ...
    ee.on(EventEmitter.errorMonitor, () => {
      events2.push("errorMonitor event");
    });
    assertThrows(() => {
      ee.emit("error");
    }, Error, "Unhandled error");
    assertEquals(events2, ["errorMonitor event"]);
    events2 = [];
    ee.on("error", () => {
      events2.push("error");
    });
    ee.emit("error");
    assertEquals(events2, ["errorMonitor event", "error"]);
  }
});
Deno.test({
...
Deno.test({
  name: "error thrown in processing loop of asynchronous event prevents processing of additional events",
  async fn() {
    const ee = new EventEmitter();
    const _err = new Error("kaboom");
    setTimeout(() => {
