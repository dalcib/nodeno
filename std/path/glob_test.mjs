...
import {SEP} from "./mod.mjs";
function match(glob2, path, opts = {}) {
  if (opts.os == null) {
    const matchDarwin = path.match(globToRegExp(glob2, {...opts, os: "darwin"}));
    if (matchDarwin) {
      assertEquals(matchDarwin.length, 1);
    }
    const matchLinux = path.match(globToRegExp(glob2, {...opts, os: "linux"}));
    if (matchLinux) {
      assertEquals(matchLinux.length, 1);
    }
    const matchWindows = path.match(globToRegExp(glob2, {...opts, os: "windows"}));
    if (matchWindows) {
      assertEquals(matchWindows.length, 1);
    }
    return !!matchDarwin && !!matchLinux && !!matchWindows;
  } else {
    const match2 = path.match(globToRegExp(glob2, opts));
    if (match2) {
      assertEquals(match2.length, 1);
    }
    return !!match2;
  }
}
...
Deno.test({
  ...
  fn() {
    const glob2 = "\\/$^+.()=!|,.*";
    assert(match(glob2, glob2));
    assert(match(glob2, glob2, {extended: false}));
  }
});
Deno.test({
...
Deno.test({
  ...
  fn() {
    ...
    assertEquals("foo".match(pattern2)?.[0], void 0);
    assertEquals("bar".match(pattern2)?.[0], void 0);
...
Deno.test({
  ...
  fn() {
    ...
    assertEquals("foo".match(pattern2)?.[0], void 0);
    assertEquals("path/to/foo".match(pattern2)?.[0], void 0);
...
Deno.test({
  ...
  fn() {
    ...
    assertEquals("foo\\bar".match(pattern1)?.[0], void 0);
