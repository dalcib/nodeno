...
for (const pathname of tests) {
  Deno.test({
    ...
    fn: async function() {
      const prelude = await Deno.readTextFile(path.resolve(rootdir, pathname.replace(/\.wasm$/, ".json")));
      const options = JSON.parse(prelude);
      const workdir = await Deno.makeTempDir();
      await copy(path.join(testdir, "fixtures"), path.join(workdir, "fixtures"));
      try {
